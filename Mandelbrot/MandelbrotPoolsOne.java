...
public class MandelbrotPoolsOne {
    ...
    public static int[][] calculateParallelPoolCoreBlocks(ExecutorService pool, int size, double xMin, double xMax, double yMin, double yMax, int nIter, int repeats) {   // a class for parallel calculation of all threads with division into as many parts as there are cores
        ...
        ArrayList<Future<?>> futures = new ArrayList<>();   // a list of 'futures' to ensure that all calculated parts are written
        ...
        for (int t = 0; t < cores; t++) {   // loop over threads
            ...
            MandelbrotTask task = new MandelbrotTask(repeats, start, end, mandelbrot, size, xMin, xMax, yMin, yMax, nIter);
    ...
    public static void main(String[] args) throws IOException {
        ...
        ExecutorService pool = Executors.newFixedThreadPool(cores);   // initialise the pool once

        ArrayList<Double> timesCoreBlocks = new ArrayList<>();
        ArrayList<Double> timesSmallBlocks = new ArrayList<>();

        int[] blockSizesToTest = {4, 8, 16, 32, 64, 128};
        ...
        for(int i = 0; i < sizes.length; i++) {   // a test for as many parts as cores
            ...
            Mandelbrot.writeMandelbrotSet(mandelbrot,"MandelbrotPoolsOneCoreBlocks_" + sizes[i]);
        }

        double[] times = timesCoreBlocks.stream().mapToDouble(Double::doubleValue).toArray();   // convert a dynamic array to a standard list

        Mandelbrot.writeTime(sizes, times, "avgTimePoolsOneCoreBlocks");


        for (int b = 0; b < blockSizesToTest.length; b++) {   // a test for differently sized parts
            for (int i = 0; i < sizes.length; i++) {
                long startTime = System.nanoTime();

                int[][] mandelbrot = calculateParallelPoolSmallBlocks(pool, sizes[i], xMin, xMax, yMin, yMax, nIter, repeats, blockSizesToTest[b]);

                long endTime = System.nanoTime();
                double avgTime = (endTime - startTime) / (repeats * 1_000_000_000.0);
                timesSmallBlocks.add(avgTime);

                Mandelbrot.writeMandelbrotSet(mandelbrot, "MandelbrotPoolsOneBlockSize" + blockSizesToTest[b] + "_" + sizes[i]);
            }

            double[] times_blocks = timesSmallBlocks.stream().mapToDouble(Double::doubleValue).toArray();   // convert a dynamic array to a standard list

            Mandelbrot.writeTime(sizes, times_blocks, "avgTimePoolsOneBlockSize" + blockSizesToTest[b]);
        }
